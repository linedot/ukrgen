...
class test_mm(unittest.TestCase):
    ...
    def test_2vx4_vector_opa(self):
        ...
        expected_preload = [
            ...
            "a1 <- LOAD aa0 + (1*VLEN^1)+0",
            "c1 <- 0",
            "b1 <- LOAD ba0 + (1*VLEN^1)+0",
            "c2 <- 0",
            "c3 <- 0",
            "b2 <- LOAD ba0 + (2*VLEN^1)+0",
            "c4 <- 0",
            "c5 <- 0",
            "b3 <- LOAD ba0 + (3*VLEN^1)+0",
            "c6 <- 0",
            "c7 <- 0",
            "a2 <- LOAD aa0 + (2*VLEN^1)+0",
            "a3 <- LOAD aa0 + (3*VLEN^1)+0",
            "aa0 <- aa0 + (4*VLEN^1)+0",
            "ba0 <- ba0 + (4*VLEN^1)+0",
        ]
        expected_mainblock = [
            ...
            "c0 <- opa(a2, b4, c0)",
            "c1 <- opa(a3, b4, c1)",
            "b5 <- LOAD ba0 + (1*VLEN^1)+0",
            "c2 <- opa(a2, b5, c2)",
            "c3 <- opa(a3, b5, c3)",
            "b6 <- LOAD ba0 + (2*VLEN^1)+0",
            "c4 <- opa(a2, b6, c4)",
            "c5 <- opa(a3, b6, c5)",
            "b7 <- LOAD ba0 + (3*VLEN^1)+0",
            "c6 <- opa(a2, b7, c6)",
            "c7 <- opa(a3, b7, c7)",
        ]
        ...
        expected_preload_mb = [
            "a0 <- LOAD aa0 + 0",
            "b0 <- LOAD ba0 + (4*VLEN^1)+0",
            "a1 <- LOAD aa0 + (1*VLEN^1)+0",
            "b1 <- LOAD ba0 + (5*VLEN^1)+0",
            "b2 <- LOAD ba0 + (6*VLEN^1)+0",
            "b3 <- LOAD ba0 + (7*VLEN^1)+0",
            "a2 <- LOAD aa0 + (2*VLEN^1)+0",
            "a3 <- LOAD aa0 + (3*VLEN^1)+0",
            "aa0 <- aa0 + (4*VLEN^1)+0",
            "ba0 <- ba0 + (8*VLEN^1)+0",
