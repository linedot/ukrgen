...
class load_register_sequence_generator(sequence_generator):
    ...
    def initialize(self):
        ...
        self.address_register_list_position = \
                self.internal_parameters["address_register_list_start"]
    ...
    def advance(self) -> dict[str,int]:
        ...
        if ((self.offsets[areg] > self.max_offset) or
           (self.offsets[areg] > self.offset_thresholds[areg])):
            ...
            self.address_register_list_position = \
                    (self.address_register_list_position+1) % len(self.address_register_list)
